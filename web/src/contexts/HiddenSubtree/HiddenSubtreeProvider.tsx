...
import HiddenSubtreeContext, { HiddenSubtreeState } from "./HiddenSubtreeContext";
...
export default function HiddenSubtreeContextProvider({
  ...
  const notifySubscribers = useCallback(() => {
    const currentState = ignoreParent 
      ...
      if (!previousStateRef.current || !areStateEqual(currentState, previousStateRef.current)) {
  ...
  const passiveContextValue = useMemo(() => ({
    getCurrentState: () => 
      ignoreParent ? stateRef.current : passiveContext ? mergeStates(stateRef.current, passiveContext.getCurrentState()) : stateRef.current,
  ...
  const contextValue = useMemo(() => ({
    ...
    hiddenOrBackgrounded_FIXME: providerState.backgrounded || providerState.hidden
  // eslint-disable-next-line react-hooks/exhaustive-deps
